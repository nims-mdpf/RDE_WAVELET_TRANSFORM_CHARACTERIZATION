...
def metadata_def_json_with_feature() -> Generator[str, None, None]:
    ...
    data = {
        "test_feature_meta1": {"name": {"ja": "特徴量1", "en": "feature1"}, "schema": {"type": "string"}, "_feature": True},
        "test_feature_meta2": {"name": {"ja": "特徴量2", "en": "feature2"}, "schema": {"type": "string"}, "unit": "V", "_feature": True},
        "test_feature_meta3": {
...
@pytest.fixture()
def metadata_def_json_empty() -> Generator[str, None, None]:
    """空のmetadata-def.json"""
    tasksupport_dir = pathlib.Path("data", "tasksupport")
    invoice_json_path = pathlib.Path(str(tasksupport_dir), "metadata-def.json")
    data: Optional[dict] = {}
